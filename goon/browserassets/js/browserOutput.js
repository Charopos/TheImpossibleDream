...
function linkify_fallback(text) {
  var rex =
    /((?:<a|<iframe|<img)(?:.*?(?:src="|href=").*?))?(?:(?:https?:\/\/)|(?:www\.))+(?:[^ ]*?\.[^ ]*?)+[-A-Za-z0-9+&@#/%?=~_|$!:,.;]+/gi;
  return text.replace(rex, ($0, $1) => {
    if (/^https?:\/\/.+/i.test($0)) {
...
function replaceRegex() {
  ...
  if (selectedRegex) {
    var replacedText = $(this)
      .html()
      .replace(selectedRegex[0], selectedRegex[1]);
...
function highlightTerms(el) {
  ...
  nodes.each((idx, node) => {
    ...
    $(node).remove();
    content.split(pattern).forEach((chunk) => {
...
function iconError(E) {
  setTimeout(() => {
    var attempts = $(this).data('reload_attempts');
    ...
    var src = this.src;
    this.src = null;
    this.src = src + '#' + attempts;
    $(this).data('reload_attempts', ++attempts);
...
function output(message, flag) {
  ...
  if (
    Object.hasOwn(opts, 'showMessagesFilters') &&
    !opts.showMessagesFilters['All'].show
  ) {
    //Get this filter type (defined by class on message)
    var messageHtml = $.parseHTML(message),
      messageClasses;
    if (Object.hasOwn(opts, 'filterHideAll') && opts.filterHideAll) {
      var internal = false;
      messageClasses = $(messageHtml).attr('class')
        ? $(messageHtml).attr('class').split(/\s+/)
        : false;
      if (messageClasses) {
      ...
      if (
        $(messageHtml).attr('class') ||
        $(messageHtml).children().attr('class')
      ) {
        messageClasses = $(messageHtml).attr('class').split(/\s+/);
        if ($(messageHtml).children().attr('class')) {
          messageClasses = messageClasses.concat(
            $(messageHtml).children().attr('class').split(/\s+/),
          );
        }
        var tempCount = 0;
        for (var i = 0; i < messageClasses.length; i++) {
          ...
          $.each(opts.showMessagesFilters, (key, val) => {
            //Every filter
            if (
              key !== 'All' &&
              val.show === false &&
              typeof val.match != 'undefined'
            ) {
...
function sendVolumeUpdate() {
  ...
  if (opts.updatedVolume) {
    runByond(
      '?_src_=chat&proc=setMusicVolume&param[volume]=' + opts.updatedVolume,
    );
  }
}
...
function adminMusicLoadedData(event) {
  if (
    opts.musicStartAt &&
    ($('#adminMusic').prop('duration') === Infinity ||
      opts.musicStartAt <= $('#adminMusic').prop('duration'))
  ) {
...
$(() => {
      ...
      if (!opts.noResponse) {
        ...
        internalOutput(
          '<div class="connectionClosed internal" data-count="' +
            opts.noResponseCount +
            '">You are either AFK, experiencing lag or the connection has closed.</div>',
          'internal',
        );
      }
  ...
  if (savedConfig.lineheight) {
    ...
    internalOutput(
      '<span class="internal boldnshit">Loaded line height setting of: ' +
        savedConfig.lineheight +
        '</span>',
      'internal',
    );
  }
  ...
  $('body').on('mousedown', (e) => {
    ...
    if (
      $contextMenu &&
      Object.hasOwn(opts, 'contextMenuTarget') &&
  ...
  $('body').on('keydown', (e) => {
    ...
    if (c.length == 0) {
      ...
      runByond(
        'byond://winset?mapwindow.map.focus=true;mainwindow.input.text=' + c,
      );
      return false;
    } else {
      runByond('byond://winset?mapwindow.map.focus=true');
  ...
  $('.sub, .toggle').mouseenter(() => {
    opts.suppressSubClose = true;
  });

  $('.sub, .toggle').mouseleave(() => {
    opts.suppressSubClose = false;
  });

  $('#decreaseFont').click((e) => {
    savedConfig.fontsize =
  ...
  $('#decreaseLineHeight').click((e) => {
    savedConfig.lineheight = Math.max(
      parseFloat(savedConfig.lineheight || 1.2) - 0.1,
      0.1,
    ).toFixed(1);
    $('body').css({ 'line-height': savedConfig.lineheight });
    setCookie('lineheight', savedConfig.lineheight, 365);
    internalOutput(
      '<span class="internal boldnshit">Line height set to ' +
        savedConfig.lineheight +
        '</span>',
      'internal',
    );
  ...
  $('#highlightTerm').click((e) => {
    ...
    var popupContent =
      '<div class="head" style="color: white;">String Highlighting</div>' +
      '<div class="highlightPopup" id="highlightPopup" style="background-color: black;">' +
      '<div style="color: white;">Choose up to ' +
      opts.highlightLimit +
  ...
  $('#saveLog').click((e) => {
    ...
    $.ajax({
      ...
      success: (styleData) => {
        var blob = new Blob(
          [
            '<head><title>Heartbreak Chat Log</title><style>',
            styleData,
            '</style></head><body>',
            $messages.html(),
            '</body>',
  ...
  $('#saveLogTxt').click((e) => {
    if (!window.Blob) {
      output(
        '<span class="big red">This function is only supported on modern browsers. Upgrade if possible.</span>',
        'internal',
      );
      return;
    }
    ...
    internalOutput(
      '<span class="internal boldnshit">Log file saved.</span>',
      'internal',
    );
  });

  $('body').on('keyup', '#highlightColor', () => {
  ...
  $('body').on('submit', '#highlightTermForm', (e) => {
    e.preventDefault();
  ...
  $('#toggleCombine').click((e) => {
    ...
    internalOutput(
      '<span class="internal boldnshit">Toggled combine to: ' +
        opts.messageCombining +
        '</span>',
      'internal',
    );
  });
